...
export const PostAuthSchema = z.object({
  ...
  v3User: z.string().openapi({ example: 'nonemonitor' }),
  v3AuthProtocol: z.enum(['md5', 'sha']).openapi({ example: 'sha' }),
  v3AuthKey: z.string().openapi({ example: 'authKey123' }),
  v3PrivProtocol: z.enum(['aes', 'des']).openapi({ example: 'aes' }),
  v3PrivKey: z.string().openapi({ example: 'privKey123' }),
  v3Level: z
