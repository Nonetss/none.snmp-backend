...
import type { getResourceFuzzySearchRoute } from './search.route';

export const getResourceFuzzySearchHandler: RouteHandler<
  typeof getResourceFuzzySearchRoute
> = async (c) => {
  const { name, page, pageSize } = c.req.valid('query');
  ...
  try {
    const data = await db
      ...
      .where(ilike(hrSWInstalledEntryTable.hrSWInstalledName, `%${name}%`));
    ...
    console.error('[Resource Fuzzy Search] Error:', error);
