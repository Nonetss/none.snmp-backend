...
export const getServiceFuzzySearchHandler: RouteHandler<
  typeof getServiceFuzzySearchRoute
> = async (c) => {
  ...
  try {
    const data = await db
      ...
      .where(ilike(hrSWRunEntryTable.hrSWRunName, `%${name}%`));
    ...
    console.error('[Service Fuzzy Search] Error:', error);
